...
def solve_cutting_stock_with_splicing(parts_dict, stock_length, kerf=0.003):
    ...
    for i in range(num_stock):
        solver.Add(sum(x[i, j] * (processed_parts[j] + kerf) for j in range(len(processed_parts))) <= y[i] * (stock_length + kerf))
    ...
    if status == pywraplp.Solver.OPTIMAL or status == pywraplp.Solver.FEASIBLE:
        ...
        for i in range(num_stock):
            if y[i].solution_value() > 0.5:
                scheme = sorted([processed_parts[j] for j in range(len(processed_parts)) if x[i, j].solution_value() > 0.5])
...
for length, count in list(st.session_state.needs.items()):
    ...
    col_a.write(f"- 零件长度: {length:.3f}m | 数量: {count} 个")
...
# 运行计算
if st.button("🚀 运行最优规划"):
    with st.spinner('正在进行复杂计算，请稍候（最长需2分钟）...'):
        report = []
        ...
        for scheme, count in best['方案详情'].items():
            # 格式化输出，强制保留3位小数
            formatted_scheme = " + ".join([f"{i:.3f}m" for i in scheme])
            st.write(f"模式 [{formatted_scheme}]: 需执行 {count} 次")
